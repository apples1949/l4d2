...
bool IsSpraysFile(const char[] fileName) {
	...
	return StrContains(fileName, ".dat", false) == length - 4 || StrContains(fileName, ".ztmp", false) == length - 4;
}

bool IsLogFile(const char[] fileName) {
	int length = strlen(fileName);
	return StrContains(fileName, ".log", false) == length - 4;
}

bool IsDemoFile(const char[] fileName) {
	int length = strlen(fileName);
	return StrContains(fileName, ".dem", false) == length - 4;
}
